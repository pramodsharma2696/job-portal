<div>
    <div class="container mx-auto py-8 px-4">
        ...
        <form wire:submit.prevent="update" class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
            <div class="flex flex-col md:flex-row">
                ...
                <div class="w-full md:w-1/2 p-8 space-y-6 border-r border-gray-200">
                    <div class="pb-4 border-b border-gray-200">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Update the job requirements and description</p>
                    </div>
                ...
                <div class="w-full md:w-1/2 p-8 space-y-6 bg-gray-50">
                    <div class="pb-4 border-b border-gray-200">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Update your company information</p>
                    </div>
                    ...
                    <div>
                        <label for="new_company_logo" class="block text-sm font-medium text-gray-700 mb-1">Company Logo</label>
                        <div class="mt-1 flex items-center">
                            <label for="new_company_logo" class="cursor-pointer">
                                ...
                                <input wire:model="new_company_logo" type="file" id="new_company_logo" class="sr-only">
                            </label>
                            @if($new_company_logo)
                                <span class="ml-4 text-sm text-gray-600">{{ $new_company_logo->getClientOriginalName() }}</span>
                            @endif
                        </div>
                        @error('new_company_logo') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        @if($company_logo)
                            <div class="mt-4">
                                <p class="text-sm text-gray-500 mb-1">Current Logo:</p>
                                <img src="{{ asset($company_logo) }}" class="h-20 w-20 object-contain rounded-lg border border-gray-200">
                            </div>
                        @endif
                        @if($new_company_logo)
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 mb-1">New Logo Preview:</p>
                                <img src="{{ $new_company_logo->temporaryUrl() }}" class="h-20 w-20 object-contain rounded-lg border border-gray-200">
                            </div>
                        @endif
                    </div>
